...
# # Get geographical information based on IP address using the GeoIP database
...
# Get geographical information based on IP address using the GeoIP database
def get_geo_info(ip_address, hashed_ip, salt):
    geo_info = {}  # Initialize geo_info to prevent UnboundLocalError
    try:
        with Reader(settings.GEOIP_PATH) as reader:
            response = reader.city(ip_address)
            geo_info = {
                ...
                'hashed_ip': hashed_ip,
                'salt': salt,
                'region': response.continent.name if response.continent else 'Unknown',
                ...
                # Uncomment below fields if needed
                # 'country_iso_code': response.country.iso_code,
                # 'state_iso_code': response.subdivisions.most_specific.iso_code,
                # 'postal_code': response.postal.code,
                # 'latitude': response.location.latitude,
...
#         # Create a new HourlyVisit entry
...
#     source_dictionary=SourceDictionary.objects.get_or_create(visitor_data['source_from'])
#     medium_dictionary=MediumDictionary.objects.get_or_create(visitor_data['medium_from'])
#     campaign_dictionary=CampaignDictionary.objects.get_or_create(visitor_data['Coming_from'])

#     source = Source.objects.get_or_create(dictionary_source=source_dictionary)[0] if source_dictionary else None
#     medium = Medium.objects.get_or_create(dictionary_medium=medium_dictionary)[0] if source_dictionary else None
#     campaign = Campaign.objects.get_or_create(dictionary_campaign=campaign_dictionary)[0] if source_dictionary else None


#     hour, _ = Hour.objects.get_or_create(hour=timezone.now() )
...
#    Hour= HourlyVisit.objects.get_or_create(place_name=place_name_region)[0] if place_name_region else None
...
#     # declaration = models.ForeignKey(Declaration, on_delete=models.CASCADE,  related_name='hourly_visits')
...
# def Hourly_Visit(visitor_data, user,date):
...
# def is_returning_visitor(hashed_ip, browser_fingerprint, user_cookie, device_type, os_family,threshold=4):
...
#     for stored in stored_visitors:
#         score = 0

#         # التحقق من بصمة المتصفح
#         if visitor_data['browser_fingerprint'] == stored['browser_fingerprint']:
#             score += 1

#         # التحقق من ملفات تعريف الارتباط (Cookies)
#         if visitor_data['cookie'] == stored['cookie']:
#             score += 1

#         # التحقق من نوع الجهاز
#         if visitor_data['device_type'] == stored['device_type']:
#             score += 1

#         # التحقق من الـ IP المشفر
#         if visitor_data['geo_info']['hashed_ip'] == stored['geo_info']['hashed_ip']:
#             score += 1

#         # التحقق من البريد الإلكتروني
#         if visitor_data.get('email') == stored.get('email'):
#             score += 1

#         # إذا كانت النقاط تتجاوز الحد المطلوب، يعتبر الزائر معروفًا
#         if score >= threshold:
#             return "الزائر معروف وقد زار الموقع من قبل."
...
# from geoip2.database import Reader
# from django.conf import settings
# from .models import UserVisit, Device, OperatingSystem, Country, Date
...
# # Get geographical information based on IP address
...
#             }
#     except Exception as e:
#         print(f"Error retrieving geo info: {e}")
#     return geo_info
...
#     salt = generate_salt()
#     hashed_ip = hash_ip(ip_address, salt)
#     device_type, os_name, browser = get_device_info(request)
    
#     # Get or create device and OS entries
#     device, _ = Device.objects.get_or_create(name=device_type)
#     os, _ = OperatingSystem.objects.get_or_create(name=os_name)

#     # Get or create country
#     geo_info = get_geo_info(ip_address)
#     country, _ = Country.objects.get_or_create(name=geo_info['country'])

#     # Get or create date
#     date, _ = Date.objects.get_or_create(date=timezone.now())

#     # Create or update user visit
...
# # Function to collect visitor data from request and compile into a dictionary
# def RecordVisitor(request):
...
#     # Retrieve the IP address and generate salt and hashed IP
#     ip_address = '66.242.64.1'  # Placeholder IP for demonstration; use get_client_ip(request) for live data
#     salt = generate_salt()
#     hashed_ip = hash_ip(ip_address, salt)
...
#     user = check_or_create_user(hashed_ip,browser_fingerprint, user_cookie,device_type,os_family)
...
# def check_or_create_user(hashed_ip, browser_fingerprint, user_cookie, device_type, os_family, email=None):
...
#     user, created = UserVisit.objects.get_or_create(
#         hashed_ip=hashed_ip,
#         defaults={
#             'browser_fingerprint': browser_fingerprint,
#             'user_cookie': user_cookie,
...
#     if not created:
#         # If the user already exists, update the existing record if necessary
#         user.browser_fingerprint = browser_fingerprint
#         user.user_cookie = user_cookie
#         user.os_family = os_family
